...
<div style="border-color: #222D32;" class="box">
  ...
  switch ($group) {
    case "1":
        $query=mysqli_query($db,"SELECT *, debtors.id AS did FROM debtors LEFT JOIN clients ON debtors.client = clients.id LEFT JOIN statuses ON debtors.status = statuses.id LEFT JOIN users ON debtors.kam = users.id");
        break;
    case "2":
        $query=mysqli_query($db,"SELECT *, debtors.id AS did FROM debtors LEFT JOIN clients ON debtors.client = clients.id LEFT JOIN statuses ON debtors.status = statuses.id LEFT JOIN users ON debtors.kam = users.id");
        break;
    case "3":
         $query=mysqli_query($db,"SELECT *, debtors.id AS did FROM debtors LEFT JOIN clients ON debtors.client = clients.id LEFT JOIN statuses ON debtors.status = statuses.id LEFT JOIN users ON debtors.kam = users.id WHERE kam = '$id'");
        break;
    case "4":
         $query=mysqli_query($db,"SELECT *, debtors.id AS did FROM debtors LEFT JOIN clients ON debtors.client = clients.id LEFT JOIN statuses ON debtors.status = statuses.id LEFT JOIN users ON debtors.kam = users.id WHERE debtors.status = '7'");
        break;
    case "5":
         $query=mysqli_query($db,"SELECT *, debtors.id AS did FROM debtors LEFT JOIN clients ON debtors.client = clients.id LEFT JOIN statuses ON debtors.status = statuses.id LEFT JOIN users ON debtors.kam = users.id WHERE kam = '$id'");
        break;
...
<tr id="<?php echo $row["did"]; ?>">
                  <td><input type="checkbox" class="emp_checkbox" data-did="<?php echo $row["did"]; ?>"></td>
                   ...
                   <td><a href="<?php echo 'singledebtor.php?id='.$row['did']?>"><?php echo $row['name'];?>
                 ...
                 if($row['status'] == '6'){
                              echo '<span class="label label-danger">Dodgy</span>';   
                        }else{
                            echo $row['type'];
                        }
                      ?>
